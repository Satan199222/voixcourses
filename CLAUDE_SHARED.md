...
Tous les SaaS Groupe J partagent **un seul projet Sanity "groupe-j"** (`SANITY_PROJECT_ID` / `SANITY_DATASET=production`).

Le champ `organizationId` permet de distinguer les articles par SaaS (ex: `megahote`, `pronopro`, `archicollab`). `null` = blog de la plateforme partagée.

### Variables d'environnement requises

Disponibles dans Doppler namespace `GROUPEJ` (dev/stg/prd) :

```
SANITY_PROJECT_ID=<id du projet groupe-j>
...
export default {
  ...
  fields: [
    ...
    { name: 'organizationId', type: 'string' }, // null = blog plateforme
...
### Conventions de nommage et requêtes GROQ

```groq
// Tous les articles d'un SaaS
*[_type == "blogPost" && organizationId == $orgId && defined(publishedAt)] | order(publishedAt desc)

// Articles de la plateforme
*[_type == "blogPost" && organizationId == null && defined(publishedAt)] | order(publishedAt desc)
```
...
- Le Studio Sanity est déployé sur Vercel sous `studio.groupe-j.com` ou via Sanity Manage
